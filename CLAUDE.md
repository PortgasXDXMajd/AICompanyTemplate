...
- **Clean Markdown.** Every `.md` file you write must be free of markdownlint warnings: a language on every code fence, compact tables, blank lines around headings, lists, fences and tables (`markdown` skill). A hook runs `scripts/mdfix.py` after each write and repairs what it can; whatever it reports back, you fix in the same turn.
