...
5. **Definition of done.** Checkable conditions, including how it will be demoed.
...
3. **Code projects: get the plans.** First make the project readable, because the adviser may not install or check out anything: default branch checked out, clean, pulled, dependencies installed with the project's install command. Then delegate to `senior-technical-adviser` with a brief: spec to plans for `specs/NNN-short-name.md` in `projects/<name>`. It writes one plan per employee-sized task under `plans/<project>/`, each with an owner. On a project with no code yet it returns only the scaffold plan; delegate again after that is merged. If it returns questions, put them to the CEO and delegate again. If the adviser's role file is missing, run step 5 of `new-project` first. If it exists but cannot be delegated to, ask the CEO to restart `claude`; do not substitute a general-purpose subagent.
...
5. Append a line to `context/log.md`, log any overruled recommendation in `context/decisions.md`, and commit with `Spec NNN: <title>`, including the new files under `plans/`.

Then route each plan to its owner in the order the plan index gives: one plan in the project's working tree at a time, and only when the plans it depends on are merged. A plan marked `unowned` follows the rule in `CLAUDE.md`: the Chief of Staff executes it on a branch `chief-of-staff/<slug>`, or proposes the hire if the role passes the four tests in `hire`. Every implementation is reviewed and refinement-checked as `REVIEW.md` describes.
