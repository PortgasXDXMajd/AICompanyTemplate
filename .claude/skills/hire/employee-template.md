...
- If `plans/<project>/PROFILE.md` exists, the codebase was imported and its existing code sets the standard. Read the parts of that profile your task touches, find the closest existing example of what you are about to build, and follow its shape: file locations, naming, error handling, test style. Do not reformat, rename or tidy code outside your task. Failures listed in the import baseline are not yours to fix unless the brief says so.
