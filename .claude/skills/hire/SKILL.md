...
- **Model:** `opus`. The CEO's standing rule is that every hire runs on the newest Opus model; the `opus` alias always points at it, so never pin a dated model ID. The only exception is the Senior Technical Adviser, which runs on `fable` at `effort: max` and is created by the `new-project` or `import-project` skill, not here. Change a hire's model only if the CEO asks.
