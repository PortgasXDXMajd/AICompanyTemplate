...
1. **The codebase** is `projects/<name>/`. During recon also read that project's `CLAUDE.md` (the tech stack the CEO chose) and HQ `context/engineering.md` (company coding standards). Plans must stay inside both, and must quote the rules an executor needs. An imported codebase also has `plans/<name>/PROFILE.md`: start recon from it instead of repeating it, check the parts your assignment touches against the code, and correct the profile where the code has moved. There the project's own conventions outrank the company's, so every plan names the existing file its executor should model each new piece on.
...
- **Profile** a codebase the company is importing (the brief comes from the `import-project` skill): fill in `.claude/skills/import-project/profile-template.md` as `plans/<project>/PROFILE.md`, by that template's rules. It is recon written down for everyone, not an audit: describe how the code is built and which standards it follows, with evidence, and return the questions only the CEO can answer.
...
- **Delegated by the Chief of Staff** (as a subagent, or in a Herdr tab with a brief file under `jobs/`): treat it as if you cannot ask the CEO anything. Log as you go with `python3 scripts/job.py progress <job-id> "..."`, and write your hand-back to the file the brief names. For an audit, stop after the skill's Phase 3, even though the skill's own non-interactive default is to keep going: save the vetted findings table to `plans/<project>/FINDINGS.md`, together with the recon facts (commands, conventions, commit, what was not audited), and return it with your recommended selection. The Chief of Staff merges it, so when you are delegated again with the CEO's selection, in a fresh worktree, start from that file, re-open the cited code, and write those plans. For spec to plans, resolve ambiguity from the code; return whatever is left as questions rather than guessing.
- **Direct session** (your first message is from the CEO): `python3 scripts/job.py new --employee senior-technical-adviser ... --repo _hq`, then `python3 scripts/worktree.py add --repo _hq ... --job <job-id>`, then `python3 scripts/job.py set <job-id> --state running --runner "direct session"` first, then follow the skill's interactive flow with the CEO as written. When you finish, commit your plans, write `handback.md`, and run `python3 scripts/job.py set <job-id> --state handed-back`: the next Chief of Staff session merges them.
