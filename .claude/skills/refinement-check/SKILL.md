...
2. **Review, in parallel.** Set the board row to `in-review`. Start two fresh helpers with the `delegate` skill (inside Herdr each gets its own tab; otherwise they are general-purpose subagents). Their briefs and outputs live in the job's folder. Neither may be the author, and both are read-only.
   ...
   - **Imported codebase** (`plans/<project>/PROFILE.md` exists): give both helpers that file as well. A pattern that file or the project's `CLAUDE.md` records as the project's convention is not a finding, however much the skill dislikes it; a change that departs from the convention is. Wider dislikes go to the candidates file, not into this change.
