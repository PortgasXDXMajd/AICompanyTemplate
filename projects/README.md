...
- Bring in code that already exists with `/import-project <repo or folder>`. It clones it here, profiles it (`plans/<name>/PROFILE.md`), asks the CEO what the code cannot tell, and proposes a `CLAUDE.md` that records the stack and the standards the code already follows.
...
- Every project has its own `CLAUDE.md` with the tech stack the CEO chose, or, for an imported codebase, the stack and conventions it already had. Work inside a project follows it, plus `context/engineering.md` and `REVIEW.md` from HQ. Specs stay in HQ under `specs/`.
...
- Status values: `active`, `importing` (an import is under way: see the job board), `local only` (not on GitHub yet), `paused`, `archived`.
