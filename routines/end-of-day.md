...
1. **Settle the board.** Run the `job-status` skill. Every open job gets a true state and a next step. Nothing is left `running` that is not running.
...
7. **Close the journal.** Append a short summary to today's journal: done, open, blocked, first thing tomorrow. Add one line to `context/log.md`.
8. **Commit** HQ by path: `context/`, `jobs/`, `ROADMAP.md`, the instruction files you changed, `.claude/agent-memory/`. Message: `End of day YYYY-MM-DD`.
