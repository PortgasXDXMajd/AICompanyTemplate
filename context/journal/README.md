...
Append an entry, with the time, whenever:
...
Format: `- HH:MM what happened (job id or file path)`. Facts, not prose. The end-of-day routine reads these files to find the lessons of the day; `context/log.md` keeps only the one-line record of finished work.
